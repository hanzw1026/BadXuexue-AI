...
class KnowledgeBaseDialog(QDialog, Ui_Dialog):
    ...
    def _get_model(self):
        ...
        if self.model is None:
            ...
            if os.path.exists(model_path) and os.path.exists(config_file):
                ...
                model = SentenceTransformer('BAAI/bge-small-zh', device='cpu')
    ...
    def _add_files(self, file_paths):
        ...
        progress_dialog.update_thread.stop()
        progress_dialog.update_thread.wait()
        ...
        try:
            ...
            if platform.system() == 'Darwin':
                ...
                if os.path.exists(tesseract_path):
                    ...
                    if init_config.isInTestMode:
                        print(f"🔧 设置 Tesseract: {tesseract_path}")

                # 设置 poppler 路径（pdf2image 需要）
                poppler_path = "/opt/homebrew/bin"
                if os.path.exists(poppler_path):
                    # 方法1：通过环境变量
                    os.environ["PATH"] = poppler_path + ":" + os.environ.get("PATH", "")
                    # 方法2：直接传给 convert_from_path
            ...
            if init_config.isInTestMode:
                ...
                if "poppler" in str(e).lower():
                    print("💡 请确保在 PyCharm 的 Terminal 中运行，或添加环境变量")
            return ""
...
class IndexProgressDialog(QDialog):
    ...
    def __init__(self, total_files, parent=None):
        ...
        self.update_thread.update_signal.connect(self._on_update_ui)  # 连接信号
        self.update_thread.start()

        self.is_cancelled = False

    def _on_update_ui(self, seconds, blink):
        """✅ 这个会被线程的 update_signal 调用！"""
        if init_config.isInTestMode:
            print(f"🔄 UI 更新: {seconds}秒, 闪烁={blink}")  # 调试
...
class UIUpdateThread(QThread):
    ...
    def run(self):
        ...
        while self.running:
            ...
            print(f"线程调试信号更新: {self.elapsed_seconds}秒, 闪烁={self.blink_state}")  # 调试
            # ✅ 每秒发送信号，触发 _on_update_ui
